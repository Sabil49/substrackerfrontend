...
function formatBillingCycle(
  ...
  // Return first letter of lowercase version: M, W, Y
  return billingCycle.charAt(0).toUpperCase();
}
...
export default function SubscriptionCard({
  ...
  const isSilent =
    subscription.isSilent || (!subscription.lastReviewedAt && daysUntil > 30);
  ...
  const getValueScoreBadge = () => {
    switch (subscription.valueScore) {
      case "worth-it":
        return {
          label: "✓ Worth it",
          color: colors.badge.worthIt,
          bg: colors.badge.worthItBg,
        };
      case "overpriced":
        return {
          label: "! Overpriced",
          color: colors.badge.overpriced,
          bg: colors.badge.overpricedBg,
        };
      case "unused":
        return {
          label: "○ Unused",
          color: colors.badge.unused,
          bg: colors.badge.unusedBg,
        };
      default:
        return null;
    }
  ...
  // Safe name initial
  const nameInitial = (
    subscription.name?.trim().charAt(0) || "?"
  ).toUpperCase();
      ...
      >
        ...
        >
          {isSilent && !subscription.isCanceled && (
            <LinearGradient
              colors={
                colors.gradient.silent as readonly [string, string, ...string[]]
              }
              style={styles.badge}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
          ...
          {subscription.isCanceled && (
            <View
              style={[styles.badge, { backgroundColor: colors.text.disabled }]}
            >
          ...
          <View style={styles.header}>
            ...
            >
              <Text style={styles.iconText}>{nameInitial}</Text>
            </LinearGradient>
          ...
          <View style={styles.footer}>
            ...
            {valueBadge && (
              <View
                style={[styles.valueScore, { backgroundColor: valueBadge.bg }]}
              >
                <Text
                  style={[styles.valueScoreText, { color: valueBadge.color }]}
                >
                  {valueBadge.label}
                </Text>
              </View>
            ...
            {subscription.usageCount !== undefined &&
              subscription.usageCount > 0 && (
                <View style={styles.usageIndicator}>
                  ...
                  />
                  <Text
                    style={[styles.usageText, { color: colors.text.secondary }]}
                  >
